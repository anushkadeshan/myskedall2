...
//faz com que o app android saiba que está fora do site
function abrePaginaExterna(origem, url){gravarCookie('planoz_externo', origem); window.location = url;}
...
function limpaString(str)
  {
  var str1 = 'áàãâäéèêëíìîïóòõôöúùûüçÁÀÃÂÄÉÈÊËÍÌÎÏÓÒÕÖÔÚÙÛÜÇ';
...
//Só passa digitação de numeros (funcao auxiliar par ao JQuery)
...
function formatar(src, mask)
  ...
  function valida_hora(str)
    ...
    cons = true; 
    if (isNaN(hora) || isNaN(minu)){alert("Preencha a hora somente com números."); str.value = ""; str.focus(); return false;}
    if (hora < 00 || hora > 23){cons = false;} 
    if (minu < 00 || minu > 59 ){cons = false;} 
    if (pont =! ':' ){cons = false;} 
    if (cons == false){alert("A hora inserida não é válida: " + str.value); str.value = ""; str.focus(); return false;} 
    return true;
    }
