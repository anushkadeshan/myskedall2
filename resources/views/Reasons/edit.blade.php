...
<div class="container-fluid mt-3">
    ...
    <div class="content">
        ...
        <div class="box box-primary">
            <div class="box-body">
                 <hr>
                <form action="{{url('admin/space-reasons-update')}}/{{$reason->id}}" method="post">
                    <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>{{ __('msg.reason') }} </label><strong style="color:red">{{$errors->first('reason')}}</strong>
                                <input type="text" class="form-control" name="reason" value="@if(old('reason')){{old('reason')}}@elseif(!empty($reason->reason)){{$reason->reason}} @endif">
                            </div>
                        </div>
                    </div>
                    <div class="row">
