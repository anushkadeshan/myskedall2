            ...
            <div class="flex-grow h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="form-input block bg-gray-50 focus:bg-white w-full rounded-md pl-10 py-2 transition ease-in-out duration-150 sm:text-sm sm:leading-5" placeholder="{{__('Search Table')}}" />
                ...
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        @foreach($this->columns as $index => $column)
                            ...
                            <div class="w-24 py-4 flex justify-center overflow-hidden align-top px-6 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider focus:outline-none">
                                <div style="color: white; background-color:green" class="px-3 py-1 rounded @if(count($selected)) bg-green-400 @else bg-gray-400 @endif text-white text-center">
                                    {{ count($selected) }}
                                </div>
                            </div>
                            @else
                    ...
                    <div class="table-row divide-x divide-blue-200 bg-blue-100">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                <div class="overflow-hidden align-top bg-blue-100 px-2 py-2 border-b border-gray-200 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                    <div></div>
                                    <div>
